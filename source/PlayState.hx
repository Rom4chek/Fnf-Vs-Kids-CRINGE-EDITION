...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Fucking loser', 0.4], //From 20% to 39%
		['You bad', 0.5], //From 40% to 49%
		['Bruh', 0.6], //From 50% to 59%
		['Ugh', 0.69], //From 60% to 68%
		['Ummm', 0.7], //69%
		['Nice', 0.8], //From 70% to 79%
		['Cool', 0.9], //From 80% to 89%
		['Chad!', 1], //From 90% to 99%
		['GIGA CHAD!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	//video
	public var videoend:Bool = false;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'alleywaygod': //i ate ur doorframe
				GameOverSubstate.deathSoundName = 'fnf_eye_loss_sfx';
				GameOverSubstate.loopSoundName = 'gameOver-god';
				GameOverSubstate.endSoundName = 'gameOverEnd';
				GameOverSubstate.characterName = 'bf-eye-dead';
		...
		dad1 = new Character(-150, 150, 'tall');
	...
	case 'alleywaygod':
		...
		if (SONG.player2 == 'god')
			{
				godT = new FlxTrail(dad, null, 5, 7, 0.3, 0.001);
				insert(members.indexOf(dadGroup) - 1, godT);
			}
		...
		sus = new FlxSprite().loadGraphic(Paths.image('FUCKYOU'));
		sus.setGraphicSize(Std.int(FlxG.width * susSizeInterval), Std.int(FlxG.height * susSizeInterval));
		sus.updateHitbox();
		sus.screenCenter();
		sus.alpha = 0;
		add(sus);
		...
		// jumpscare
		jumpScare = new FlxSprite().loadGraphic(Paths.image('jumpscare'));
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'keith':
					startVideo('intro');
					case 'from-fnf':
						startDialogue(dialogueJson, 'DialogueTALL');
						case 'too-fat':
							startDialogue(dialogueJson, 'DialogueFAT');
							case 'stupid-kids-drama':
								startDialogue(dialogueJson, 'DialogueDRAMA');
			...
			{
				case 'ommetaphobia':
					startDialogue(dialogueJson, 'LunchboxScary');
		...
		if(foundFile) {
			...
			(new FlxVideo(fileName)).finishCallback = function() {
				...
				if(endingSong) {
					...
					//startCountdown();
					startDialogue(dialogueJson, 'DialogueSHIT');
				}
			}
			return;
		} else {
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(tmr.loopsLeft % 2 == 0) {
					...
					if (dad1.animation.curAnim != null && !dad1.animation.curAnim.name.startsWith('sing') && !dad1.stunned)
						{
							dad1.dance();
						}
						if (dad2.animation.curAnim != null && !dad1.animation.curAnim.name.startsWith('sing') && !dad2.stunned)
							{
								dad2.dance();
							}
				}
				...
				else if(dad1.danceIdle && dad1.animation.curAnim != null && !dad1.stunned && !dad1.curCharacter.startsWith('gf') && !dad1.animation.curAnim.name.startsWith("sing"))
					{
						dad1.dance();
					}

					else if(dad2.danceIdle && dad2.animation.curAnim != null && !dad2.stunned && !dad2.curCharacter.startsWith('gf') && !dad2.animation.curAnim.name.startsWith("sing"))
						{
							dad2.dance();
						}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				swagNote.gfAltNote = (section.gfSection && (songNotes[1]<4));
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						sustainNote.gfAltNote = (section.gfSection && (songNotes[1]<4));
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'How many times bf cum: ' + songScore + ' | Skill issues: ' + songMisses + ' | Ur personality: ' + ratingName;
		} else {
			scoreTxt.text = 'How many times bf cum: ' + songScore + ' | Skill issues: ' + songMisses + ' | Ur personality: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
		...
		if(SONG.song == 'Ommetaphobia' && songMisses > 68) {
				health = 0;
				trace("Defeat reference lol");
				camZooming = false; //lol
			}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Remove two kids':
				remove(dad1);
				remove(dad2);

			case 'Transformation':
				if(dad.animOffsets.exists('Summon')) {
					dad.playAnim('Summon', true);
					dad.specialAnim = true;
			}
			    if(dad1.animOffsets.exists('Summon')) {
			    	dad1.playAnim('Summon', true);
			    	dad1.specialAnim = true;
		    }
		        if(dad2.animOffsets.exists('Summon')) {
		    	dad2.playAnim('Summon', true);
	...
	var susSizeInterval:Float = 1.120;
	var jumpscareSizeInterval:Float = 1.120;
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week1_beat', 'beat_ommeta', '100',
				'ommetaphob_fc', 'dodge', 'week7_nomiss', 'ur_bad',
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(daNote.noteType == 'Dodge Note') {
				 ...
				 if(boyfriend.animOffsets.exists('hurt')) {
					...
					#if ACHIEVEMENTS_ALLOWED
				Achievements.dodgeMiss++;
				FlxG.save.data.dodgeMiss = Achievements.dodgeMiss;
				var achieve:String = checkForAchievement(['dodge']);
				if (achieve != null) {
					startAchievement(achieve);
				} else {
					FlxG.save.flush();
				}
				FlxG.log.add('Misses: ' + Achievements.dodgeMiss);
				#end
			}
		}

		if(daNote.noteType == 'Dodge Sussy Note') {
			sus.alpha += 0.100;
			if(boyfriend.animOffsets.exists('hurt')) {
			   boyfriend.playAnim('hurt', true);
			   boyfriend.specialAnim = true;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			{
				if (SONG.notes[curSection].altAnim || note.noteType == 'Alt Animation' || note.noteType == 'GF alt Sing') {
					altAnim = '-alt';
				}
			}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				if(note.noteType == 'Dodge Sussy Note') {
					if(boyfriend.animOffsets.exists('dodge')) {
						boyfriend.playAnim('dodge', true);
						boyfriend.specialAnim = true;
						boyfriend.heyTimer = 0.2;
	...
	override function beatHit()
	{
		...
		if (curBeat % gfSpeed == 0) {
			curBeat % (gfSpeed * 2) == 0 ? {
				iconP1.scale.set(1.4, 0.5);
				iconP2.scale.set(1.4, 1.6);

				  FlxTween.angle(iconP1, -30, 0, Conductor.crochet / 1000 * gfSpeed, {ease: FlxEase.quadOut});
				  FlxTween.angle(iconP2, 30, 0, Conductor.crochet / 1000 * gfSpeed, {ease: FlxEase.quadOut});
				} : {
					iconP1.scale.set(1.4, 1.6);
					iconP2.scale.set(1.4, 0.5);
	
					FlxTween.angle(iconP2, -30, 0, Conductor.crochet / 1000 * gfSpeed, {ease: FlxEase.quadOut});
					FlxTween.angle(iconP1, 30, 0, Conductor.crochet / 1000 * gfSpeed, {ease: FlxEase.quadOut});
				}
	
				FlxTween.tween(iconP1, {'scale.x': 1, 'scale.y': 1}, Conductor.crochet / 1050 * gfSpeed, {ease: FlxEase.quadOut});
				FlxTween.tween(iconP2, {'scale.x': 1, 'scale.y': 1}, Conductor.crochet / 1050 * gfSpeed, {ease: FlxEase.quadOut});
		...
		if(curBeat % 2 == 0) {
			...
			if (dad1.animation.curAnim.name != null && !dad1.animation.curAnim.name.startsWith("sing") && !dad1.stunned)
				{
					dad1.dance();
				}
		...
		else if(dad1.danceIdle && dad1.animation.curAnim.name != null && !dad1.curCharacter.startsWith('gf') && !dad1.animation.curAnim.name.startsWith("sing") && !dad1.stunned) {
			dad1.dance();
		}
		else if(dad2.danceIdle && dad2.animation.curAnim.name != null && !dad2.curCharacter.startsWith('gf') && !dad2.animation.curAnim.name.startsWith("sing") && !dad2.stunned) {
			dad2.dance();
		}
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			// Rating FC
			ratingFC = "";
			if (sicks > 0) ratingFC = "wow u are very sick Fc";
			if (goods > 0) ratingFC = "well good fc i think";
			if (bads > 0 || shits > 0) ratingFC = "bruh it's just fc";
			if (songMisses > 0 && songMisses < 10) ratingFC = "damn u sucker";
			else if (songMisses >= 10) ratingFC = "well atleast u can beat it";
		}
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case '100':
						if(isStoryMode && songMisses < 100 && storyPlaylist.length <= 1 && !usedPractice) {
							unlock = true;
						}
					case 'beat_ommeta':
						if(Paths.formatToSongPath(SONG.song) == 'ommetaphobia' && !usedPractice) {
							unlock = true;
						}
					case 'ommetaphob_fc':
						if(Paths.formatToSongPath(SONG.song) == 'ommetaphobia' && songMisses < 1 && !usedPractice) {
							unlock = true;
						}
					case 'ur_good':
